var website = new (function WLCWebsite() {
	...
	this.popupWindowsService = new (function () {
		...
		function PopupWindow(rootElement, options) {
			...
			this.show = function (showBackplate) { _show.call(this, showBackplate); }
			...
			this.refresh = function () {
				// not implemented yet
				// to be overrided
				// console.log('Refreshing '+this.logName+' content. This is the default method.');
			}
				...
				for (var i = 0; i < elementsArray.length; i++) {
					...
					if ( wlcJS.domTools.isDom(element) ) {
						...
						if (!elementAlreadyInOneOfTheArrays) {
						    //console.log(this.logName + ': adding ', element, 'to', targetArrayName);
			...
			function _removeButtonsFromArray(elementOrArrayToAdd, targetArray) {
				...
				//console.log('_removeButtonsFromArray();');
			...
			function _show(showBackplate) {
				if (this.isShown) {
					// console.log(this.logName+' has already been opened. Skipped.');
					return false;
				}
				this.isShown = true;
				...
				_showBackplate = (typeof showBackplate === 'undefined') ? this.options.showBackplate : showBackplate;
				_popupWindowsService.showPopupWindow(this, _showBackplate);
				...
				// console.log(this.logName+'this.options.autoHide:', this.options.autoHide, '\nthis.options:', this.options)
				if (this.options.autoHide) {
                    ...
                    //console.log(this.logName + ' will close automatically in about ' + Math.round(_duration / 1000) + ' seconds.');
			...
			function _hide() {
				if (!this.isShown) {
					// console.log(this.logName+' has already been closed. Skipped.');
					return false;
				}
				this.isShown = false;
