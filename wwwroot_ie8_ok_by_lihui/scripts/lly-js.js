var wlcJS = new (function () {
	...
	function WLC_DOMTools() {
		...
		function _addDom ( tagName, classNames, id, uri ) {
			...
			if (window.id(id)) { console.warn('Element of id ['+id+'] already exists!');
			}
		...
		Element.prototype.centerTo = function (options) {
			...
			if (_.alongX) {
			    ...
			    if (_.centerRef === window) {
			        var refWidth = window.innerWidth;
			    } else {
                    var refWidth = _.centerRef.clientWidth;
			    }
				//console.log('refWidth:',refWidth,'px\n','selfWidth:',selfWidth,'px\n','_.offsetX:',_.offsetX,'px\n');
				var left = undefined;
				...
				if (_.shrinkWhenNeeded) {
					...
					if (typeof this.computedWidthBeforeCenterTo === 'undefined') {
						...
						this.computedWidthComesFromInlineDefinition = (this.style.width.length > 0);
					} else {
			...
			if (_.alongY) {
				...
				if (_.shrinkWhenNeeded) {
					...
					if (typeof this.computedHeightBeforeCenterTo === 'undefined') {
						...
						this.computedHeightComesFromInlineDefinition = (this.style.height > 0);
					} else {
						// always try to use original value
